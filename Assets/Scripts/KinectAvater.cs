...
public class KinectAvater : MonoBehaviour {
    ...
    // 自分の関節とUnityちゃんのボーンを入れる用
    // TODO : 精度上げるには制御する部分の数増やせばいいのでは？
	...
	void Start () {
        ...
        Spine1 = Hips.transform.Find("Character1_Spine").gameObject.
            transform.Find("Character1_Spine1").gameObject;
	...
	void Update () {
        ...
        // デリゲート（Body型b.IsTracked）の中で最初の要素をbodyに代入している
        Body body = data.FirstOrDefault(b => b.IsTracked);
        if(body == null)
        {
            return;
        }
